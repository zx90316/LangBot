...
class PluginManager:
    ...
    async def _launch_plugin_installation_via_ws(
        self,
        binding: InstallationBinding,
        runtime: PluginInstallationRuntime,
        capability: str,
        *,
        new_plugin_connection_callback: typing.Callable[
            [Connection], typing.Coroutine[typing.Any, typing.Any, None]
        ],
    ) -> None:
        """Launch one installation worker over WebSocket (Windows stdio is unsupported)."""
        ...
        worker_env = _oss_dev_worker_environment(
            {
                PLUGIN_FILE_STORAGE_DIR_ENV: str(
                    (runtime.paths.root_path / "rpc-transfer").resolve()
                ),
                PLUGIN_REGISTRATION_CAPABILITY_ENV: capability,
                PLUGIN_RUNTIME_PROFILE_ENV: "oss_dev",
            }
        )
        worker_env["RUNTIME_WS_URL"] = (
            f"ws://localhost:{self.context.ws_debug_port}/plugin/ws"
        )

        process: asyncio.subprocess.Process = await asyncio.create_subprocess_exec(
            pkgmgr_helper.get_plugin_python(str(runtime.artifact.code_path)),
            "-m",
            "langbot_plugin.cli.__init__",
            "run",
            "--prod",
            env=worker_env,
            cwd=str(runtime.artifact.code_path),
        )
        try:
            await process.wait()
        finally:
            ...
            if getattr(process, "returncode", 0) is None:
                ...
                if not stopped:
                    logger.error(
                        "Windows plugin installation worker did not exit after SIGKILL: %s",
        ...
        *,
        ...
        if enabled:
            ...
            async with self._installation_lifecycle_semaphore():
                ...
                if getattr(self.context, "runtime_profile", "oss_dev") == "oss_dev":
                    code_path = str(artifact.code_path)
                    requirements_file = os.path.join(code_path, "requirements.txt")
                    if os.path.exists(requirements_file):
                        returncode, output = (
                            await pkgmgr_helper.install_requirements_isolated(
                                code_path
                            )
                        )
                        if returncode != 0:
                            await self._stop_installation_worker(current)
                            current.state = "failed"
                            current.error_code = "dependency_install_failed"
                            current.error_message = output.strip() or (
                                "Plugin dependency installation failed"
                            )
                            logger.error(
                                "Plugin dependency installation failed for "
                                "installation %s: %s",
                                binding.installation_uuid,
                                current.error_message,
                            )
    ...
    async def launch_plugin_installation(
        ...
        try:
            async def new_plugin_connection_callback(
            ...
            if get_platform() == "win32":
                await self._launch_plugin_installation_via_ws(
                    binding,
                    runtime,
                    capability,
                    new_plugin_connection_callback=new_plugin_connection_callback,
                )
            else:
                ...
                async def stdio_connection_callback(connection: Connection):
                    await new_plugin_connection_callback(
                        connection,
                        controller.process,
                    )

                await controller.run(stdio_connection_callback)
        except asyncio.CancelledError:
            logger.info(
